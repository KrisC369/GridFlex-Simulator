...
mvn -q clean
mvn -q compile
nice -n 19 mvn exec:java -Dexec.mainClass=be.kuleuven.cs.flexsim.experimentation.$TARGET -Dexec.classpathScope=runtime -Dexec.cleanupDaemonThreads=false -Dexec.args="-r $REPS -n $AGENTS -s $SOLVER -m $MODE -p1start $P1START -p1step $P1STEP -p1end $P1STOP -dIdx $ERRORPROF -pIdx $DATAPROF -distribution $DIST $EXTRAOPTS"
